...
export async function PUT(req: NextRequest) {
  ...
  await supabase.from("customer_profiles").upsert({ user_id: session.user.id, full_name: fullName, phone, updated_at: new Date().toISOString() });
  return NextResponse.json({ profile: { id: data.user.id, email: data.user.email, ...data.user.user_metadata, full_name: fullName, phone } });
}
